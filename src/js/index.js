...
const links = [
  {
    ico:'<i class="fab fa-font-awesome"></i>',
    name: `fontawesome`,
    data: 0,
  },
  {
...
class Index {
  ...
  showhLibrary(params) {
    ...
    this.page = 1;
    this.pageCounts = 0;
    this.counts % this.pageEnd > 0 ? this.pageCounts = this.moves + 1 : this.pageCounts = this.moves;
  ...
  createSearchbar() {
    ...
    input.oninput = () => {
      if (input.value === '') {
        ...
        this.searching = true;
        this.founds == 0 ? document.getElementById('action-page').classList.add('hidden') : document.getElementById('action-page').classList.remove('hidden');
      }
    }
  ...
  createLi(categ, icn) {
    ...
    iconBtn.onclick = () => {
      if (!iconBtn.classList.contains('copied')) {
        ...
        setTimeout(() => {
          iconBtn.innerHTML = `copy <i class="far fa-copy"></i>`;
          iconBtn.classList.remove('bg-green-500', 'hover:bg-green-500', 'copied')
        }, 800);
      }     
    }
  ...
  paginateSearch(counts) {
    ...
    counts % this.pageEnd > 0 ? this.pageCounts = this.moves + 1 : this.pageCounts = this.moves;
